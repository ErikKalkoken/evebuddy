...
func (a *inventoryTypeInfo) update(ctx context.Context) error {
	et, err := a.iw.u.EVEUniverse().GetOrCreateTypeESI(ctx, a.typeID)
	if err != nil {
		return err
	}
	fyne.Do(func() {
	...
	if et.HasRender() {
		a.typeIcon.OnTapped = func() {
			a.iw.showZoomWindow(et.Name, a.typeID, a.iw.u.EVEImage().InventoryTypeRenderAsync, a.iw.w)
		}
	}

	var character *app.EveEntity
	if a.characterID != 0 {
		ee, err := a.iw.u.EVEUniverse().GetOrCreateEntityESI(ctx, a.characterID)
		if err != nil {
			return err
		}
		character = ee
	...
	oo, err := a.iw.u.EVEUniverse().ListTypeDogmaAttributesForType(ctx, et.ID)
	if err != nil {
		return err
	}
...
func (a *inventoryTypeInfo) calcAttributesData(ctx context.Context, et *app.EveType, attributes map[int64]*app.EveTypeDogmaAttribute) []typeAttributeRow {
	...
	for _, ag := range attributeGroups {
		...
		for _, o := range attributeSelection {
			...
			v, substituteIcon := a.iw.u.EVEUniverse().FormatDogmaValue(ctx, value, o.DogmaAttribute.Unit)
			var iconID int64
			if substituteIcon != 0 {
	...
	if v, ok := et.Volume.Value(); ok {
		value, _ := a.iw.u.EVEUniverse().FormatDogmaValue(ctx, v, app.EveUnitVolume)
		if pv, ok := et.PackagedVolume.Value(); ok && !optional.Equal(et.Volume, et.PackagedVolume) {
			s, _ := a.iw.u.EVEUniverse().FormatDogmaValue(ctx, pv, app.EveUnitVolume)
...
func (a *inventoryTypeInfo) calcFittingData(ctx context.Context, dogmaAttributes map[int64]*app.EveTypeDogmaAttribute) []typeAttributeRow {
	...
	for _, da := range attributeGroupsMap[attributeGroupFitting] {
		...
		v, _ := a.iw.u.EVEUniverse().FormatDogmaValue(ctx, o.Value, o.DogmaAttribute.Unit)
...
func (a *inventoryTypeInfo) addJanicePriceItems(ctx context.Context, typeID int64) ([]attributeItem, error) {
	j, err := a.iw.u.Janice().FetchPrices(ctx, typeID)
	if err != nil {
...
func (a *inventoryTypeInfo) calcRequiredSkills(ctx context.Context, characterID int64, attributes map[int64]*app.EveTypeDogmaAttribute) ([]requiredSkill, error) {
	...
	for i, x := range skillAttributes {
		...
		et, err := a.iw.u.EVEUniverse().GetType(ctx, typeID)
		if err != nil {
			return nil, err
		}
		skill := requiredSkill{
		...
		cs, err := a.iw.u.Character().GetSkill(ctx, characterID, typeID)
