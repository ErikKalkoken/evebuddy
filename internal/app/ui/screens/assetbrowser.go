...
func NewCharacterBrowser(u baseUI) *AssetBrowser {
	return newBrowser(u, false)
}

func NewCorporationBrowser(u baseUI) *AssetBrowser {
	return newBrowser(u, true)
}

func newBrowser(u baseUI, forCorporation bool) *AssetBrowser {
	a := &AssetBrowser{
...
func (w *navItem) CreateRenderer() fyne.WidgetRenderer {
	c := container.NewBorder(nil, nil, nil, w.count, w.name)
	return widget.NewSimpleRenderer(c)
}

func (w *navItem) set(name string, count optional.Optional[int]) {
	w.name.SetText(name)
	var s string
...
func (a *browserNavigation) setFooter(s string, i widget.Importance) {
	a.footer.Text = s
...
func (a *browserLocation) set(cn *containerNode) {
	...
	case asset.NodeLocation:
		...
		a.info.OnTapped = func() {
			a.selected.ab.u.InfoViewer().ShowLocation(el.ID)
		}
		a.info.Show()
	case asset.NodeAsset:
		a.info.OnTapped = func() {
...
func (w *assetItem) CreateRenderer() fyne.WidgetRenderer {
	c := container.NewPadded(container.New(layout.NewCustomPaddedVBoxLayout(0),
		container.New(&xlayout.BottomRightLayout{}, w.icon, w.badge),
		w.label,
	))
	return widget.NewSimpleRenderer(c)
}
