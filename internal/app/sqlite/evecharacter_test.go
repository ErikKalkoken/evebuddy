...
func TestEveCharacter(t *testing.T) {
	...
	t.Run("can create new", func(t *testing.T) {
		...
		arg := sqlite.CreateEveCharacterParams{ID: 1, Name: "Erik", CorporationID: corp.ID, RaceID: race.ID}
		// when
	...
	t.Run("can fetch character by ID with all fields populated", func(t *testing.T) {
		...
		arg := sqlite.CreateEveCharacterParams{AllianceID: alliance.ID, FactionID: faction.ID}
