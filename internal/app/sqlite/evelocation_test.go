...
func TestLocation(t *testing.T) {
	...
	t.Run("can get existing", func(t *testing.T) {
		// given
		testutil.TruncateTables(db)
		factory.CreateLocationStructure(sqlite.UpdateOrCreateLocationParams{ID: 42, Name: "Alpha"})
		// when
		x, err := r.GetEveLocation(ctx, 42)
		// then
		if assert.NoError(t, err) {
	...
	t.Run("can update existing", func(t *testing.T) {
		// given
		testutil.TruncateTables(db)
		factory.CreateLocationStructure(sqlite.UpdateOrCreateLocationParams{ID: 42})
		owner := factory.CreateEveEntityCorporation()
		system := factory.CreateEveSolarSystem()
		myType := factory.CreateEveType()
		updatedAt := time.Now()
