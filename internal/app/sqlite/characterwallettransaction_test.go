...
func TestWalletTransaction(t *testing.T) {
	...
	t.Run("can list IDs of existing entries", func(t *testing.T) {
		...
		testutil.TruncateTables(db)
		c := factory.CreateCharacter()
		e1 := factory.CreateCharacterWalletTransaction(sqlite.CreateCharacterWalletTransactionParams{CharacterID: c.ID})
		e2 := factory.CreateCharacterWalletTransaction(sqlite.CreateCharacterWalletTransactionParams{CharacterID: c.ID})
		e3 := factory.CreateCharacterWalletTransaction(sqlite.CreateCharacterWalletTransactionParams{CharacterID: c.ID})
		// when
