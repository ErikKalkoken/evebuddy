...
func TestCharacterImplant(t *testing.T) {
	...
	t.Run("can create new", func(t *testing.T) {
		...
		arg := sqlite.CreateCharacterImplantParams{
	...
	t.Run("can replace implants", func(t *testing.T) {
		...
		testutil.TruncateTables(db)
		c := factory.CreateCharacter()
		factory.CreateCharacterImplant(sqlite.CreateCharacterImplantParams{CharacterID: c.ID})
		eveType := factory.CreateEveType()
		arg := sqlite.CreateCharacterImplantParams{
			EveTypeID:   eveType.ID,
			CharacterID: c.ID,
		...
		// when
		err := r.ReplaceCharacterImplants(ctx, c.ID, []sqlite.CreateCharacterImplantParams{arg})
		// then
		if assert.NoError(t, err) {
			x, err := r.GetCharacterImplant(ctx, c.ID, arg.EveTypeID)
			if assert.NoError(t, err) {
	...
	t.Run("can list implants", func(t *testing.T) {
		...
		testutil.TruncateTables(db)
		c := factory.CreateCharacter()
		x1 := factory.CreateCharacterImplant(sqlite.CreateCharacterImplantParams{CharacterID: c.ID})
		x2 := factory.CreateCharacterImplant(sqlite.CreateCharacterImplantParams{CharacterID: c.ID})
		// when
