...
func TestCharacter(t *testing.T) {
	...
	t.Run("can get with all dependencies", func(t *testing.T) {
		...
		eveC := factory.CreateEveCharacter(sqlite.CreateEveCharacterParams{AllianceID: a.ID, FactionID: f.ID})
		c1 := factory.CreateCharacter(sqlite.UpdateOrCreateCharacterParams{ID: eveC.ID})
		// when
