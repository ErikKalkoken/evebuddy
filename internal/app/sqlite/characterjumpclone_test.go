...
func TestCharacterJumpClone(t *testing.T) {
	...
	t.Run("can create new empty clone", func(t *testing.T) {
		...
		arg := sqlite.CreateCharacterJumpCloneParams{
			CharacterID: c.ID,
			JumpCloneID: 5,
	...
	t.Run("can replace existing clone", func(t *testing.T) {
		...
		testutil.TruncateTables(db)
		c := factory.CreateCharacter()
		factory.CreateCharacterJumpClone(sqlite.CreateCharacterJumpCloneParams{
			CharacterID: c.ID,
		})
		location := factory.CreateLocationStructure()
		eveType := factory.CreateEveType()
		...
		// when
		err := r.ReplaceCharacterJumpClones(ctx, c.ID, []sqlite.CreateCharacterJumpCloneParams{arg})
		// then
		if assert.NoError(t, err) {
			x, err := r.GetCharacterJumpClone(ctx, c.ID, 5)
			if assert.NoError(t, err) {
	...
	t.Run("can list clones", func(t *testing.T) {
		...
		testutil.TruncateTables(db)
		c := factory.CreateCharacter()
		x1 := factory.CreateCharacterJumpClone(sqlite.CreateCharacterJumpCloneParams{
			CharacterID: c.ID,
		})
		// when
