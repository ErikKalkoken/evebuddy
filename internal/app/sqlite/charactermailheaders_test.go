...
func TestListMailHeaders(t *testing.T) {
	...
	t.Run("should return mail for selected label only", func(t *testing.T) {
		...
		m1 := factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID, LabelIDs: []int32{l1.LabelID}, Timestamp: time.Now().Add(time.Second * -120)})
		m2 := factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID, LabelIDs: []int32{l1.LabelID}, Timestamp: time.Now().Add(time.Second * -60)})
		factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID, LabelIDs: []int32{l2.LabelID}})
		// when
	...
	t.Run("can fetch for all labels", func(t *testing.T) {
		...
		m1 := factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID, LabelIDs: []int32{l1.LabelID}, Timestamp: time.Now().Add(time.Second * -120)})
		m2 := factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID, LabelIDs: []int32{l1.LabelID}, Timestamp: time.Now().Add(time.Second * -60)})
		m3 := factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID, LabelIDs: []int32{l2.LabelID}, Timestamp: time.Now().Add(time.Second * -240)})
		m4 := factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID, Timestamp: time.Now().Add(time.Second * -360)})
		// when
	...
	t.Run("should return mail without label only", func(t *testing.T) {
		...
		m := factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID})
		// when
	...
	t.Run("should return mail for selected list only", func(t *testing.T) {
		...
		m1 := factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{
			CharacterID:  c.ID,
		...
		factory.CreateCharacterMail(sqlite.CreateCharacterMailParams{CharacterID: c.ID})
		// when
