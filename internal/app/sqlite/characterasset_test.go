...
func TestCharacterAsset(t *testing.T) {
	...
	t.Run("can update existing", func(t *testing.T) {
		...
		testutil.TruncateTables(db)
		c := factory.CreateCharacter()
		x1 := factory.CreateCharacterAsset(sqlite.CreateCharacterAssetParams{CharacterID: c.ID})
	...
	t.Run("can list assets in ship hangar", func(t *testing.T) {
		...
		shipCategory := factory.CreateEveCategory(sqlite.CreateEveCategoryParams{ID: app.EveCategoryShip})
		shipGroup := factory.CreateEveGroup(sqlite.CreateEveGroupParams{CategoryID: shipCategory.ID})
		shipType := factory.CreateEveType(sqlite.CreateEveTypeParams{GroupID: shipGroup.ID})
		x1 := factory.CreateCharacterAsset(sqlite.CreateCharacterAssetParams{
			CharacterID: c.ID,
			LocationID:  location.ID,
			EveTypeID:   shipType.ID,
		})
		factory.CreateCharacterAsset(sqlite.CreateCharacterAssetParams{
			CharacterID: c.ID,
			LocationID:  location.ID,
		})
		factory.CreateCharacterAsset(sqlite.CreateCharacterAssetParams{
	...
	t.Run("can delete excluded assets", func(t *testing.T) {
		...
		c := factory.CreateCharacter()
		x1 := factory.CreateCharacterAsset(sqlite.CreateCharacterAssetParams{CharacterID: c.ID})
		factory.CreateCharacterAsset(sqlite.CreateCharacterAssetParams{CharacterID: c.ID})
		// when
