...
func TestCharacterService_UpdateWalletJournalEntryESI(t *testing.T) {
	...
	t.Run("should create new entry from scratch", func(t *testing.T) {
		...
		require.NoError(t, err)
		xassert.EqualOptional(t, -100000.0, x.Amount)
		xassert.EqualOptional(t, 500000.4316, x.Balance)
		xassert.EqualOptional(t, int64(4), x.ContextID)
		xassert.EqualOptional(t, "contract_id", x.ContextIDType)
		xassert.Equal(t, time.Date(2018, 02, 23, 14, 31, 32, 0, time.UTC), x.Date)
		xassert.Equal(t, "Contract Deposit", x.Description)
		xassert.Equal(t, firstParty.ID, x.FirstParty.MustValue().ID)
		xassert.Equal(t, "contract_deposit", x.RefType)
		xassert.Equal(t, secondParty.ID, x.SecondParty.MustValue().ID)
	...
	t.Run("should not update existing", func(t *testing.T) {
		// given
		testutil.MustTruncateTables(db)
		httpmock.Reset()
		s := NewFake(Params{Storage: st})
...
func TestCharacterService_UpdateWalletTransactionESI(t *testing.T) {
	db, st, factory := testutil.NewDBOnDisk(t)
	defer db.Close()
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
...
func TestCharacterService_UpdateWalletBalanceESI(t *testing.T) {
	db, st, factory := testutil.NewDBOnDisk(t)
	defer db.Close()
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()
	...
	t.Run("should update", func(t *testing.T) {
		...
		require.NoError(t, err)
		xassert.EqualOptional(t, balance, c2.WalletBalance)
	})
}
