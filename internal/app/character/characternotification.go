...
func (s *CharacterService) CountCharacterNotificationUnreads(ctx context.Context, characterID int32) (map[evenotification.Folder]int, error) {
	types, err := s.st.CountCharacterNotificationUnreads(ctx, characterID)
	if err != nil {
		return nil, err
	}
	categories := make(map[evenotification.Folder]int)
	for name, count := range types {
		c := evenotification.Type2folder[evenotification.Type(name)]
		categories[c] += count
	}
