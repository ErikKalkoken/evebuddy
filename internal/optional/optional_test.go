...
func TestConvertNumeric(t *testing.T) {
	assert.Equal(
		t,
		optional.From(int(99)),
		optional.ConvertNumeric[int64, int](optional.From(int64(99))),
	)
	assert.Equal(
		t,
		optional.From(float64(99)),
		optional.ConvertNumeric[int32, float64](optional.From(int32(99))),
	)
	assert.Equal(
		t,
...
func TestFromIntegerWithZero(t *testing.T) {
	assert.Equal(t, optional.From(5), optional.FromIntegerWithZero(5))
	assert.Equal(t, optional.Optional[int]{}, optional.FromIntegerWithZero(0))
}

func TestFromIntegerWithTime(t *testing.T) {
	x := time.Now()
	assert.Equal(t, optional.From(x), optional.FromTimeWithZero(x))
