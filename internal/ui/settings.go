...
func makeSettingsDialog(u *ui) (*dialog.CustomDialog, error) {
	maxMails, err := u.service.Dictionary.DictionaryIntWithFallback(model.SettingMaxMails, model.SettingMaxMailsDefault)
	if err != nil {
		return nil, err
	}
	...
	maxTransactions, err := u.service.Dictionary.DictionaryIntWithFallback(model.SettingMaxWalletTransactions, model.SettingMaxWalletTransactionsDefault)
	if err != nil {
		return nil, err
	}
	...
	form := &widget.Form{
		...
		OnSubmit: func() {
			...
			u.service.Dictionary.DictionarySetInt(model.SettingMaxMails, maxMails)
			u.setTheme(themeRadio.Selected)
			u.service.Dictionary.DictionarySetString(model.SettingTheme, themeRadio.Selected)
			d.Hide()
		},
		OnCancel: func() {
			d.Hide()
